...
public:
  ...
  void Marginalize(
    ...
    const Eigen::MatrixXd H_marginalized = H_rr - H_rm*H_mm_inv*H_mr;
    const Eigen::MatrixXd b_marginalized = b_r - H_rm*H_mm_inv*b_m;

    //把H矩阵对角化，进而根据边缘化公式求得雅可比矩阵J和残差r
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes(H_marginalized);//求特征值
    ...
    Eigen::VectorXd S_inv_sqrt = S_inv.cwiseSqrt(); //R = P.cwiseProduct(Q); R = P .* Q 对应点相乘
