  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    if ( jacobians ) {
      // compute shared intermediate results:
        ...
        if ( jacobians[0] ) {
             // implement computing:
             Eigen::Map<Eigen::Matrix<double,6,15,Eigen::RowMajor>>jacobian_i(jacobians[0]);//RowMajor:行优先
             jacobian_i.setZero();
        ...
        if ( jacobians[1] ) {
            // implement computing:
            Eigen::Map<Eigen::Matrix<double,6,15,Eigen::RowMajor>>jacobian_j(jacobians[1]);//RowMajor:行优先
            jacobian_j.setZero();
