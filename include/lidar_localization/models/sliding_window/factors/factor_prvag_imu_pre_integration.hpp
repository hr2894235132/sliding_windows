...
public:
  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    //
    //LLT：Cholesky分解，把一个对称正定的矩阵表示成一个下三角矩阵L和其转置的乘积的分解
    ...
    residual.block<3,1>(INDEX_R,0) = (Sophus::SO3d::exp(theta_ij)*ori_i.inverse()*ori_j).log();
    ...
    if ( jacobians ) {
      // compute shared intermediate results:
        ...
        const Eigen::Matrix3d J_r_inv = JacobianRInv(residual.block(INDEX_R,0,3,1));//右乘雅克比
      if ( jacobians[0] ) {
          Eigen::Map<Eigen::Matrix<double,15,15,Eigen::RowMajor>>jacobian_i(jacobians[0]);//RowMajor:行优先
          jacobian_i.setZero();
        // a. residual, position:
      ...
      if ( jacobians[1] ) {
          Eigen::Map<Eigen::Matrix<double,15,15,Eigen::RowMajor>>jacobian_j(jacobians[0]);//RowMajor:行优先
          jacobian_j.setZero();
        // a. residual, position:
